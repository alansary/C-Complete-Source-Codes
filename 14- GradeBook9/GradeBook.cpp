...
void GradeBook::inputGrades() {
	...
	// .get() member function returns one character from the
	// user input
	// EOF is a representation of the fact that we have hit
	// the end of the input stream, and it is a symbolic
	// constant that is predefined in the C++ header files
	// and in particular in <iostream>
	// EOF exists in C++11 and not in C++99
	// g++ -std=c++11 *.cpp -o GradeBook
	// ./GradeBook
	// EOF sequence in Linux is Ctrl+D
	while ( (grade = cin.get()) != EOF ) {
		// determine which grade was entered
		// switch statement is used only with integer values
		// and characters
		// in switch statement, the curly braces are required
		switch (grade) { // switch statement nested in while
			...
			case 'f': // or lowercase f
				...
				break; // exit switch
			// prevent catching newline characters,
			// tabs, and spaces in the input
			case '\n': // ignore newlines,
			case '\t': // tabs,
...
// display a report based on grades entered by user
